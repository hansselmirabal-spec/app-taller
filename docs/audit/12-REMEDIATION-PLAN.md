...
**Actualización 2026-08-13 (post-auditoría, mismo día):** de los 9 bloqueantes, 5 ya se cerraron: los 4 de código (BE-01/A-3, BE-02/A-4, A-2, ID-02, FE-16 — desplegados a QAS, PR #50/#51/#52) más backup/restore (DEVOPS-02, automatizado y probado en el servidor). Quedan pendientes 3, todos por decisión/credenciales del usuario: TLS en PROD, SMTP en PROD, y aclarar la deriva de schema/datos de PROD — ver estado marcado en cada fila.

**Hallazgo adicional 2026-08-14 (post-auditoría, reportado por el usuario con evidencia):** `BudgetAppointmentsService.create()` (el flujo "Cita de Presupuesto" / Agenda de Presupuestos) tenía exactamente el mismo bug que BE-02/A-4 — cero chequeo de duplicado — pero en un tercer service que la auditoría original no cubrió (solo citó `bodyshop.service.ts` y `appointments.service.ts` como evidencia de BE-02). Confirmado en producción de datos: 6 grupos de citas duplicadas reales ya existían en QAS antes del fix. **Resuelto el mismo día** — PR #56, mismo patrón (advisory lock + índice único parcial `budget_appointments_one_active_per_slot`), datos duplicados limpiados en QAS antes de aplicar la migración. Ver `05-DATABASE-AUDIT.md` y `03-BACKEND-AUDIT.md` para contexto de BE-02/A-4 — este es su tercera instancia, ya cerrada.
...
| **Correr el pipeline de deploy a PROD antes del go-live real** | A-1 | back/arch/HM | TLS + SMTP resueltos primero (no tiene sentido promover a un PROD sin ninguno de los dos) | S | Ninguno hoy — PROD no está en uso real | ~~Confirmar si PROD está en uso~~ **Aclarado 2026-08-13 por el usuario: PROD no está en uso, todo lo probado pasa por QAS — no es una deriva inesperada, es que nunca se promovió.** Reclasificado de "bloqueante urgente" a "paso del checklist de go-live": correr `deploy-prod.yml` (aplica migraciones 007-012 vía `migrationsRun: true`) recién cuando se decida ir a producción real, junto con TLS/SMTP resueltos | ⏸ Reclasificado — no bloquea nada hoy, pendiente para cuando se decida el go-live real |
| **Automatizar y probar backup/restore de PROD** | DEVOPS-02 | devops | Ninguna | S (cron) + M (prueba de restore) | Pérdida total de datos ante corrupción o error humano | Backup diario automatizado corriendo (cron o step de CI), con retención definida; restore real ejecutado con éxito en ambiente aislado; RPO/RTO documentados | ✅ Resuelto — 2026-08-13. Cron diario (PROD+QAS, retención 14d) + restore real probado (30 tablas, conteos verificados, 4s). RPO≈24h, RTO≈segundos |
