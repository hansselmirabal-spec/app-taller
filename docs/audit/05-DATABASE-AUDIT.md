...
**Aclarado por el usuario (2026-08-13):** PROD todavía no está en uso real — todo lo probado hasta hoy pasó por QAS. Esto es esperado, no un bug de sincronización: PROD simplemente no fue promovido todavía. Deja de ser "deriva de schema inesperada" y pasa a ser un ítem de checklist de go-live: correr el pipeline de deploy a PROD (que aplica migraciones pendientes vía `migrationsRun: true`) recién cuando se decida ir a producción real — no antes, y no como un fix aislado de esta auditoría. Baja de severidad de facto (no bloquea nada hoy), pero sigue siendo parte del gate de go-live junto con TLS/SMTP.
