...
export function Sidebar() {
  ...
  const inSystemSettings = pathname.startsWith('/settings/users') || pathname.startsWith('/settings/roles');
  const inSettings = pathname.startsWith('/settings') && !inSystemSettings;
  const [settingsOpen, setSettingsOpen] = useState(inSettings);
  const [systemSettingsOpen, setSystemSettingsOpen] = useState(inSystemSettings);
  ...
  const navSystemSettings = [
    { href: '/settings/users', label: 'Usuarios y Permisos', icon: Users },
  ...
  return (
    <aside className="flex h-screen flex-col bg-white border-r border-slate-200" style={{ width: 'var(--sidebar-width)', minWidth: 'var(--sidebar-width)' }}>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ...
        {isUserAdmin && <div className="pt-2">
          <button
            onClick={() => setSystemSettingsOpen(o => !o)}
            className={cn(
              'w-full flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-all duration-100',
              inSystemSettings ? 'text-blue-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
            )}
          >
          ...
          {systemSettingsOpen && (
            <div className="mt-0.5 ml-3 pl-4 border-l border-slate-200 space-y-0.5">
              {navSystemSettings.map(({ href, label, icon: Icon }) => {
                const active = pathname === href;
                return (
                  <Link
                    key={href}
