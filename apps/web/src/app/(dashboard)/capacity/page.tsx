...
export default function CapacityPage() {
  ...
  // Contadores globales de la semana
  ...
  // "Completo": técnico con ≥90% de su capacidad usada (sin llegar a sobrecarga)
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="grid border-b border-slate-200 bg-slate-50" style={{ gridTemplateColumns: '200px repeat(6, 1fr)' }}>
              ...
              {weekDates.map(d => {
                ...
                return (
                  <div key={d.toISOString()} className="px-3 py-3 text-center border-r border-slate-200 last:border-r-0">
                    <p className={`text-xs font-medium capitalize ${isToday ? 'text-blue-600' : 'text-slate-500'}`}>
                      {format(d, 'EEE', { locale: es })}
                    </p>
                    <p className={`text-base font-bold mt-0.5 ${isToday ? 'text-blue-700' : 'text-slate-900'}`}>
                      {format(d, 'd')}
                    </p>
                    {isToday && <div className="h-1 w-1 rounded-full bg-blue-500 mx-auto mt-1" />}
                  </div>
                );
              })}
            </div>
            ...
            {technicians.map((tech, idx) => (
              ...
              >
                ...
                {weekDates.map(d => {
                  ...
                  const dayCapacity = weekCapacity[dateStr] ?? [];
                  const cap = dayCapacity.find(c => c.technicianId === tech.id);
                  if (!cap) return (
                  ...
                  return (
                    <div key={dateStr} className={`px-2 py-3 border-r border-slate-100 last:border-r-0 ${isToday ? 'bg-blue-50/30' : ''}`}>
                      <button
                        onClick={() => setAbsenceTarget({ technicianId: tech.id, technicianName: tech.name, date: dateStr })}
                      ...
                      >
                        ...
                        {status === 'overloaded' && (
                          <p className="text-xs mt-0.5 opacity-90">+{(cap.usedHours - cap.availableHours).toFixed(1)}h extra</p>
                        )}
                        {(status === 'available' || status === 'partial') && (
                          <p className="text-xs mt-0.5 opacity-80">{remaining.toFixed(1)}h libres</p>
                        )}
                        {status === 'occupied' && (
                          <p className="text-xs mt-0.5 opacity-80">Sin cupos</p>
                        )}
                        {status !== 'off' && status !== 'absence' && cap.availableHours > 0 && (
                          <div className="mt-1.5 h-1 bg-current opacity-20 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${Math.min((cap.usedHours / cap.availableHours) * 100, 100)}%` }}
                            />
                          </div>
                        )}
                        {status === 'absence' && (
                          <p className="text-xs mt-0.5 opacity-80">
                            {cap.absenceType === 'half' ? 'Media jornada' : 'Feriado'}
                          </p>
                        )}
                      </button>
                    </div>
